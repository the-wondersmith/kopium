...
pub struct Container {
    ...
    /// Extracted documentation properties for the container
...
impl Container {
    ...
    pub fn can_derive_default(&self, containers: &[Container]) -> bool {
        ...
        for m in &self.members {
            // If the type contains a <, it's a container type. All kopium containers (Map, Vec, Option) impl Default.
...
impl Container {
    ...
    pub fn builder_fields(&mut self) {
        ...
        {
            let mut annotation = String::from("#[builder(default");

            // avoid name collisions in the code generated by `typed-builder`
            if field.name == "build" {
            ...
            field.extra_annot.push(annotation);
        }
    }
...
impl Output {
    ...
    ///
    /// It is unsound to skip this step. Some CRDs use kebab-cased members, which are invalid in Rust.
